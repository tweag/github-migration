...
def analyze_gitlab_group(server_url, private_token, group_name, output_file=None):
    ...
    if not output_file:
        ...
        output_file = f'gitlab_size_analysis_{group_name.replace("/", "_")}_{timestamp}.csv'
    ...
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        ...
        writer.writerow([
            ...
            'Last Active', 'Web URL', 'wiki_size (B)', 'package_size (B)', 'lfs_objects_size (B)', 'container_registry_size (B)'
        ])
        
        projects = list(group.projects.list(iterator=True, include_subgroups=True, all=True))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='GitLab Repository Size Analyzer (Statistics Endpoint)')
    parser.add_argument('--server', required=True, help='GitLab server URL (e.g., https://gitlab.com)')
    parser.add_argument('--token', required=True, help='Personal access token with read_api and read_repository scopes')
